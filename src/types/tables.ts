...
export type table<T, R> = {
    ...
    include: <K extends keyof Utils.includeArrays<T>, S, r>(entity: K, q: (t: table<Utils.getKeysFromArray<T, K>, Utils.Unit>) => table<S, r>) => table<Omit<T, K>, { K: r[] } & R>,
...
export type lazyTable<T1, T2, R> = {
    ...
    include: <K extends keyof Utils.includeArrays<T2>, S, r>(entity: K, q: (t: table<Utils.getKeysFromArray<T2, K>, Utils.Unit>) => table<S, r>) => lazyTable<T1, Omit<T2, K>, { K: r[] } & R>
    orderby: <K extends keyof R>(order: ('ASC' | 'DESC'), by: K) => lazyTable<T1, T2, R>
    apply: (data: T1[]) => R[]
}
