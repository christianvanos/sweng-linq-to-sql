...
export const Identity = <a>(): Func<a, a> => Func(x => x)


const repeat = function <a>(f: Func<a, a>, n: number): Func<a, a> {
	if (n <= 0) {
...
const repeatUntil = function<a>(f: Func<a, a>, predicate: Func<a, boolean>) : Func<a, a> {
	const g =
      (x: a) => {
      	if (predicate.f(x)) {
