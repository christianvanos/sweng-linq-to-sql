...
const generateCourses = (amount: number): List<Course> => {
	...
	for (let i = 0; i <= getRandomInt(0, 15); i++) {
		courses = Cons(
			Course(
...
export const DataGenerator = (length: number): List<Student> => {
	...
	const names = ['Steven', 'Bob', 'Ellen', 'Blake', 'Johny', 'Keith', 'Brett', 'Brad', 'Carie', 'Chris', 'Miranda', 'Kane']
	const surnames = ['Degeneres', 'Doest', 'Koerts', 'Shelton', 'Cash', 'Eldredge', 'Paisley', 'Young', 'Metz', 'Underwood', 'Lambert', 'Pitt', 'Brown', 'Depp', 'Jansen', 'van Vliet', 'Williams']
	const gender: Array<'male' | 'female'> = ['male', 'female']
