...
const ListOperations = <T>(): ListOperations<T> => ({
	...
	sort: function (this: List<T>, order: Comparers = 'ASC'): List<T> {
		...
		const middle = Math.floor(this.count() / 2 -1)
		const p = this.splitAt(middle)
		const left = p.First.sort(order)
		const right = p.Second.sort(order)
